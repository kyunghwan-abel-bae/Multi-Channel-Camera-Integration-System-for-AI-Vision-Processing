...
class RunnableRGBStreamingSignal(QObject):
    """ A class to declare signals for RunnableRGBStreaming
    """
    streaming_done = Signal(str)


class RunnableRGBStreaming(QRunnable):
    """ A base class is for devices.
    it supports a RGB color streaming
    ...
    def __init__(self, device_info, output, device_name, device_id, is_landscape=True):
        super(RunnableRGBStreaming, self).__init__()
        ...
        self.__vid_writer = cv2.VideoWriter("Video.avi", cv2.VideoWriter_fourcc(*'MJPG'), 30, self.__resolution,
                                            isColor=True)

        self.signal_ = RunnableRGBStreamingSignal()
    ...
    def get_rgb_array_from(self, arg_data, arg_width, arg_height):
        ''' calculate a rgb version of np.array
        ...
        '''
        data = np.frombuffer(arg_data, dtype=np.uint8, offset=0)
        data_rgb = data.reshape(arg_height, arg_width)
    ...
    def run_stream(self, cam=0, pdata=0, ndatasize=0):
        ...
        try:
            while True:
                ...
                ret = cam.MV_CC_GetOneFrameTimeout(pdata, ndatasize, st_frame_info, 5000)

                if ret == 0:
                    frame = cast(pdata, POINTER(c_ubyte))

                    if PixelType_Gvsp_Mono8 == st_frame_info.enPixelType:
                        num_array = self.get_rgb_array_from(pdata, st_frame_info.nWidth, st_frame_info.nHeight)
                    else:
                        num_array = None

                    if self.is_vision_processing() is True:
                        num_array = self.process_frame(num_array)

                    if num_array is None:
                        qim = QImage()
                    else:
                        qim = QImage(num_array.data, num_array.shape[1], num_array.shape[0], QImage.Format_RGB888)
                        if self.record_on():
                            if current_record_on_status == False:
                                ...
                                resized_num_array = cv2.resize(num_array, dsize=self.resolution(),
                                                               fx=self.ratio_record(), fy=self.ratio_record(),
